...
async def get_forum_topics(client: TelegramClient, username: str) -> list[Topic]:
    ...
    # Telegram may include deleted topics (ForumTopicDeleted) in the
    # response. These have no title or top_message, so they must be
    # skipped rather than treated as regular topics.
    topics = [
        Topic(
            topic_id=raw.id,
            title=raw.title,
