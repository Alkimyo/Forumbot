...
async def main() -> None:
    """Start the health-check server, Telethon userbot, and polling loop.

    Runs a lightweight HTTP server (for platforms like Render's Web
    Service tier that require a bound port) concurrently with the
    Telethon userbot session and the aiogram long-polling loop.
    """
